<template>
  <q-card>
    <q-card-section class="row">
      <div class="text-h6">Add Public Meeting's Reader</div>
      <q-space />
      <q-btn flat round dense icon="close" v-close-popup />
    </q-card-section>

    <template v-if="numberOfPublicMeetingReaders">
...
export default {
  ...
  methods: {
    ...mapActions("publicMeetingReaders", ["addMeeting"]),
    submitForm() {
  ...
  computed: {
    ...mapGetters("publishers", [
      ...
      "numberOfPublicMeetingReaders"
    ]),
    publishersOptions: function() {
      let options = [];
      let publishers = this.publicMeetingReaders;
