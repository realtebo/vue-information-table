<template>
  <q-card>
    ...
    <template v-if="numberOfAssignedToSoundDepartment">
      <q-form @submit.prevent="submitForm">
        <q-card-section>
          ...
          <q-select
            v-if="
              meetingToSubmit.who3 != null || noOfPeopleAtSoundDepartment > 2
            "
            class="q-mb-sm"
            outlined
...
export default {
  ...
  methods: {
    ...mapActions("soundDepartment", ["updateMeeting", "deleteMeeting"]),
    submitForm() {
      this.updateMeeting({
        id: this.id,
